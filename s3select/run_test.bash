...
{
l="$*"  
res=$( echo 1 | ./s3select -q  "select ${l} from stdin;" ) 
echo $res | sed 's/.$//'
}
...
echo check sum 
echo $(./generate_rand_csv 10 10 | ./s3select -q 'select sum(int(_1)) from stdin;') $(./generate_rand_csv 10 10 | awk 'BEGIN{FS=",";} {s+=$1;} END{print s;}')
echo check min 
echo $(./generate_rand_csv 10 10 | ./s3select -q 'select min(int(_1)) from stdin;') $(./generate_rand_csv 10 10 | awk 'BEGIN{FS=",";min=100000;} {if(min>$1) min=$1;} END{print min;}' )
echo check max 
echo $(./generate_rand_csv 10 10 | ./s3select -q 'select max(int(_1)) from stdin;') $(./generate_rand_csv 10 10 | awk 'BEGIN{FS=",";max=0;} {if(max<$1) max=$1;} END{print max;}' )
echo check substr and count 
echo  $(./generate_rand_csv 10000 10 | ./s3select -q 'select count(int(_1)) from stdin where int(_1)>200 and int(_1)<250;') \
$(./generate_rand_csv 10000 10 | ./s3select -q 'select substr(_1,1,1) from stdin where int(_1)>200 and int(_1)<250;' | uniq -c | awk '{print $1;}')
